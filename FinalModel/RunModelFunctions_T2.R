...
RunModel_RA_T2 <- function(AllocationMethod, EstimationMethod, n_max, n_min, n_step, n0, Z, Results, k, p_known, params){
...
RunModel_CARA_T2 <- function(AllocationMethod, EstimationMethod,  n_max, n_min, n_step, n0, Z, Results, k, p_known, params){
  c_gamma = params$c_gamma
...
RunModel_ML_T2 <- function(AllocationMethod,  n_max, n_min, n_step, n0, Z, Results, k, p_known, params){
  temp = params$temp
  eps = params$eps
...
RunModel_Gittins_T2 <- function( n_max, n_min, n_step, n0, Z, Results, k, p_known, Random, GI){
...
runSims_T2 = function(k,  n_max, n_min, n_step, n0, SuccessProbs, params, GI) {
  ...
  for (AM in Model_allocations) {
    for (EM in Model_estimations) {
      df = rbind(df, RunModel_RA_T2(AllocationMethod = AM, EstimationMethod = EM,  n_max, n_min, n_step, n0, Z, Results, k, p_known = SuccessProbs, params), deparse.level = 2)
      df = rbind(df, RunModel_CARA_T2(AllocationMethod = AM, EstimationMethod = EM,  n_max, n_min, n_step, n0, Z, Results, k, p_known = SuccessProbs, params), deparse.level = 1)
    }
  ...
  for (M in ML_Allocations) {
    df= rbind(df, RunModel_ML_T2(M,  n_max, n_min, n_step, n0, Z, Results, k, SuccessProbs_known, params), deparse.level = 1)
  }
  
  # Run gittins
  df= rbind(df, RunModel_Gittins_T2( n_max, n_min, n_step, n0, Z, Results, k, SuccessProbs, TRUE, GI), deparse.level = 1)
  df= rbind(df, RunModel_Gittins_T2( n_max, n_min, n_step, n0, Z, Results, k, SuccessProbs, FALSE, GI), deparse.level = 1)
