...
RunModel_RA_T5 <- function(AllocationMethod, EstimationMethod, n_max, n_min, n_step, n0, Z, Results, k, p_known, params){
  ...
  for(j in n_s){
    ...
    if(sum(Z[1:j]==0) ==0){
      Z0_TotalSuccesses = 0
      Z0_TotalTreatmentSuccesses = c(0,0,0,0,0)  
    }else{
    ...
    df_update = c(paste("RA", EstimationMethod, AllocationMethod, sep = "_"),
                  ...
                  TotalTreatmentSuccesses[3],
                  TotalTreatmentSuccesses[4],
                  TotalTreatmentSuccesses[5],
                  sum(Allocation[1:j]==1),
                  sum(Allocation[1:j]==2),
                  sum(Allocation[1:j]==3),
                  sum(Allocation[1:j]==4),
                  sum(Allocation[1:j]==5),
                  Z0_TotalSuccesses,
                  Z0_TotalTreatmentSuccesses[1],
                  Z0_TotalTreatmentSuccesses[2],
                  Z0_TotalTreatmentSuccesses[3],
                  Z0_TotalTreatmentSuccesses[4],
                  Z0_TotalTreatmentSuccesses[5],
                  sum(Allocation[1:j][Z[1:j]==0]==1),
                  sum(Allocation[1:j][Z[1:j]==0]==2),
                  sum(Allocation[1:j][Z[1:j]==0]==3),
                  sum(Allocation[1:j][Z[1:j]==0]==4),
                  sum(Allocation[1:j][Z[1:j]==0]==5),
...
RunModel_CARA_T5 <- function(AllocationMethod, EstimationMethod,  n_max, n_min, n_step, n0, Z, Results, k, p_known, params){
  c_gamma = params$c_gamma
...
RunModel_ML_T5 <- function(AllocationMethod,  n_max, n_min, n_step, n0, Z, Results, k, p_known, params){
  temp = params$temp
  eps = params$eps
...
RunModel_Gittins_T5 <- function( n_max, n_min, n_step, n0, Z, Results, k, p_known, Random, GI){
...
runSims_T5 = function(k,  n_max, n_min, n_step, n0, SuccessProbs, params, GI) {
  ...
  df = data.frame(
    ...
    Success_TreatmentC = integer(),
    Success_TreatmentD = integer(),
    Success_TreatmentE = integer(),
    Allocation_TreatmentA = integer(),
    Allocation_TreatmentB = integer(),
    Allocation_TreatmentC = integer(),
    Allocation_TreatmentD = integer(),
    Allocation_TreatmentE = integer(),
    Z0_Successes_total = integer(),
    Z0_Successes_TreatmentA = integer(),
    Z0_Successes_TreatmentB = integer(),
    Z0_Successes_TreatmentC = integer(),
    Z0_Successes_TreatmentD = integer(),
    Z0_Successes_TreatmentE = integer(),
    Z0_Allocation_TreatmentA = integer(),
    Z0_Allocation_TreatmentB = integer(),
    Z0_Allocation_TreatmentC = integer(),
    Z0_Allocation_TreatmentD = integer(),
    Z0_Allocation_TreatmentE = integer(),
    ChiSq_pvalue = double(),
  ...
  for (AM in Model_allocations) {
    for (EM in Model_estimations) {
      df = rbind(df, RunModel_RA_T5(AllocationMethod = AM, EstimationMethod = EM,  n_max, n_min, n_step, n0, Z, Results, k, p_known = SuccessProbs, params), deparse.level = 2)
      df = rbind(df, RunModel_CARA_T5(AllocationMethod = AM, EstimationMethod = EM,  n_max, n_min, n_step, n0, Z, Results, k, p_known = SuccessProbs, params), deparse.level = 1)
    }
  ...
  for (M in ML_Allocations) {
    df= rbind(df, RunModel_ML_T5(M,  n_max, n_min, n_step, n0, Z, Results, k, SuccessProbs_known, params), deparse.level = 1)
  }
  
  # Run gittins
  df= rbind(df, RunModel_Gittins_T5( n_max, n_min, n_step, n0, Z, Results, k, SuccessProbs, TRUE, GI), deparse.level = 1)
  df= rbind(df, RunModel_Gittins_T5( n_max, n_min, n_step, n0, Z, Results, k, SuccessProbs, FALSE, GI), deparse.level = 1)
