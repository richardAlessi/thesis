...
df_check_T5 <- df_T5 %>% group_by(Model, Sample_size, Success_Probabilities, Run) %>% summarise(rowCount = n())
unique(df_check_T5$rowCount)

```

```{r}
df_check_T5[df_check_T5$rowCount == 20 & df_check_T5$Model == "RA_ML Model_EpsilonGreedy" & df_check_T5$Sample_size == 100,]	
...
df_check_T2 <- df_T2 %>% group_by(Model, Sample_size, Success_Probabilities) %>% summarise(rowCount = n())
unique(df_check_T2$rowCount)

```

```{r}
# Scenario 5 analysis
df5_T5 = df_T5[df_T5$Success_Probabilities == "c(0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4)"]


df5_check_T5 <- df5_T5 %>% group_by(Model, Success_Probabilities) %>% summarise(rowCount = n())
unique(df5_check_T5$rowCount)
